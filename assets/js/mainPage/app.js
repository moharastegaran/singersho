...
jQuery(document).ready(function($){
  ...
  $(window).on("load",function () {
      if(localStorage.getItem("accessToken")===null){
          $("header .nav-item.authenticated").addClass("d-none");
          $("header .nav-item.not-authenticated").removeClass("d-none");
      }else{
          $("header .nav-item.authenticated").removeClass("d-none");
          $("header .nav-item.not-authenticated").addClass("d-none");
          $(".auth.name").text(localStorage.getItem("auth.first_name")+" "+localStorage.getItem("auth.last_name"));
      }
  });

    $("header .nav-item.not-authenticated a.btn, .user-settings-menu a").on("click",function () {
      ...
      if(typeof _el === 'object' && _el.length > 0) {
        _el.each(function(){
          var _width  = $(this).hasClass('m-item-block') ? $(this).outerWidth() : $(this).width(),
              _left   = $(this).hasClass('m-item-block') ? $(this).parent('li').position().left : parseInt($(this).css('padding-right')) + $(this).parent('li').position().left,
              _target = $(this).parent('li');
          var _right = _target.parent().outerWidth() - (_target.position().left + _target.outerWidth());
          if($(".border-hr").hasClass("move-left")){
            _right = _right + 15;
          }
          $(this).parents('.moving-border').next('.border-hr').css({
            'width' : _width + 'px',
            'margin-right'  : _right + 'px',
          });
        });
      }
    };
